...
from matplotlib.patches import Circle # 더 이상 사용되지 않지만, 혹시 모를 경우를 대비하여 일단 유지
from PIL import Image, ImageDraw # 더 이상 사용되지 않지만, 혹시 모를 경우를 대비하여 일단 유지
import platform
...
st.markdown(
    ...
    body {
        ...
        background-image: url('https://upload.wikimedia.wikimedia.org/wikipedia/commons/thumb/c/c5/ESO_-_The_Milky_Way_over_Paranal_%28by_Y.Beletsky%29.jpg/1280px-ESO_-_The_Milky_Way_over_Paranal_%28by_Y.Beletsky%29.jpg');
...
# --- 시뮬레이션 계산 로직 (캐싱) ---
@st.cache_data
def calculate_magnification_data(lens_m, planet_m_ratio, planet_orb, phase, velocity): # impact_param 제거
    time_points = np.linspace(-15, 15, 300)
    magnification = 1.0 + np.exp(-(time_points / (50 / velocity))**2) * (lens_m * 0.5)

    if planet_m_ratio > 0: # has_p 제거, planet_m_ratio로 존재 여부 확인
        ...
        # 충격 매개변수가 0.1보다 작을 때의 밝기 증폭 로직은 더 이상 필요 없으므로 제거
        # if impact_param < 0.1:
...
def simulation_page():
    ...
    # 이미지 시각화를 삭제했기 때문에 impact_parameter의 시뮬레이션 영향도 제거됩니다.
    # 밝기 곡선 계산에 직접적인 영향을 주지 않으므로, 고정값 0.5로 대체하거나 그냥 제거해도 됩니다.
    # 여기서는 고정값으로 설정하여 calculate_magnification_data 함수의 인자 수는 유지하겠습니다.
    # 다만, 밝기 곡선 자체는 충격 매개변수에 큰 영향을 받지 않도록 모델링되어 있습니다.
    impact_parameter_fixed = 0.5 # 이미지 시각화가 없으므로 임의의 고정값 사용
    ...
    st.sidebar.write("---")
    st.sidebar.info("참고: 이 시뮬레이터의 밝기 곡선은 개념적인 모델에 기반하며, 실제 천체 물리 계산과 다를 수 있습니다.") # 설명 업데이트

    time_points, magnifications = calculate_magnification_data(
        lens_mass, planet_mass_ratio, planet_orbit_radius, planet_phase,
        lens_velocity # impact_parameter 인자 제거
    )
    ...
    # fig_image_sim = create_microlensing_image_cached(impact_parameter_fixed)
...
def explanation_page():
    ...
    **중력 마이크로렌징(Gravitational Microlensing)**은 아인슈타인의 일반 상대성 이론에 의해 예측되는 현상으로,
    ...
    광원 별의 밝기가 일시적으로 증가하는 현상을 말합니다. (이미지 왜곡 부분 삭제)
    """)
    ...
    """)
    # 충격 매개변수 설명 제거
    # st.write(" - **충격 매개변수 (Impact Parameter):** 렌즈 별과 광원 별의 시선 방향 상의 가장 가까운 거리를 나타냅니다. 이 값이 작을수록 (0에 가까울수록) 렌즈 별과 광원 별이 더 정확히 정렬되어 밝기 증가 폭이 커지고 왜곡이 심해집니다.")
